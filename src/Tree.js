      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (v_icon!=undefined)
				...
				v_a = createSimpleElement('a',null,'node_text');
		...
		renameNode: function(p_node) {
			...
			var onRename = function(e) {
				var blur = e.type === 'blur';
				var keypressEnter = e.keyCode == 13;
				
				if (blur || keypressEnter) {
					var newNodeText = document.createElement('a');
					newNodeText.setAttribute('class', 'node_text');
					newNodeText.innerText = e.target.value;
					nodeTextWrapper.appendChild(newNodeText);
					if (keypressEnter) {
						newNameInput.removeEventListener('blur', onRename);
					}
					nodeTextWrapper.removeChild(newNameInput);
				}
			}
			newNameInput.setAttribute('type', 'text');
			newNameInput.setAttribute('value', oldName);
			newNameInput.addEventListener('blur', onRename);
			newNameInput.addEventListener('keypress', onRename);
